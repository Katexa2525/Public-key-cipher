...
namespace WindowsFormsApp1
{
  public partial class Form1 : Form
  {
    ...
    private bool IsCorrectNumber(in BigInteger num)
    {
      ...
      if (num % 4 != 3)
      {
        MessageBox.Show("Должно выполняться правило: a % 4 = 3", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        return false;
      }

      if (!IsPrimary_MillerRabin(num, 10))
      {
        MessageBox.Show("Числа должны быть простыми", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        return false;
      }
    ...
    private BigInteger GetValueFromTextBox(in string name, in string source)
    {
      ...
      {
        MessageBox.Show($"Некорректное значение {name}", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        return -1;
      }
    ...
    {
      //int[] arrP = { 7, 11, 13, 19, 29, 37, 43, 47, 53, 59 };
      //int[] arrQ = { 5711, 5903, 6659, 7043, 7109, 8527, 8941, 9227, 9479, 9491 };

      //Random rnd = new Random();
      //int indP = rnd.Next(0, 9);
      //int indQ = rnd.Next(0, 9);

      ClearText();

      //PTextBox.Text = arrP[indP].ToString();
      //QTextBox.Text = arrQ[indQ].ToString();

      SizeTB.Text = "1";

      if (PTextBox.Text == "" || QTextBox.Text == "")
      {
        MessageBox.Show("Заполните p и q", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        return;
      }
      ...
      if (BTextBox.Text.Length > 0)
      {
        ...
        if (!encryptor.SetB(b))
        {
          MessageBox.Show("b должно быть меньше n", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
          return;
        }
      }
      else
      ...
      if (encryptor != null)
      {
        if (InputTB.Text.Length > 0)
        {
          ...
          */
          ...
          {
            MessageBox.Show("Нет подходящей дешифровки", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            FillOutput(result);
          }
          ...
          MessageBox.Show("Сначала необходимо заполнить входные данные", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
      }
      else
        MessageBox.Show("Сначала необходимо заполнить данные", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
    }
    ...
    private void SaveFile_Click(object sender, EventArgs e)
    {
      if (OutputTB.Text.Length > 0)
      {
        try
        {
          ...
          MessageBox.Show("Файл записан!", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        catch (Exception ex)
        {
        ...
        MessageBox.Show("Нечего сохранять", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    ...
    private void OpenFile_Click(object sender, EventArgs e)
    {
      if (openFileDialog1.ShowDialog() != DialogResult.Cancel)
      {
        ...
        byteArr = File.ReadAllBytes(_filePath);
    ...
    {
      int[] arrP = { 7, 11, 19, 43, 47, 59, 67, 71, 83, 103, 227};
      int[] arrQ = { 5711, 5903, 6659, 7043, 8527, 9227, 9479, 9491, 9539, 10343};

      Random rnd = new Random();
      int indP = rnd.Next(0, 10);
      int indQ = rnd.Next(0, 9);
      PTextBox.Text = arrP[indP].ToString();
      QTextBox.Text = arrQ[indQ].ToString();
    }
